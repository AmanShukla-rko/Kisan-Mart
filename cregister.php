...
if(isset($_POST['register'])){
    ...
    $insert_c = "insert into customers (cust_ip,cust_name,cust_email,cust_pass,cust_contact) 
                  values ('$ip','$c_name','$c_email','$c_pass','$c_contact')";
    $run_c = mysqli_query($con,$insert_c);
   $sel_cart = "select * from customers where c_email='$c_email'";
...
<hr>
    <div class="jumbotron">
    <form name="form2" action="cregister.php" method="post" enctype="multipart/form-data">

      <div class="form-row">
        <div class="form-group col-md-5">
          <label>Name</label>
          <input type="FullName" class="form-control"  placeholder="Enter Full Name" name="c_name" required>
        </div>

        <div class="form-group col-md-5">
          <label>Email ID</label>
          <input class="form-control"  placeholder="Enter Email-ID" name="c_email" onkeyup="checkEmail(this.value)" required>
        </div>

          <div class="form-group col-md-5">
          <label >Phone Number</label>
          <input type="text" class="form-control" id="number"  name="c_contact" required pattern=".*">
        </div>
          
        <div class="form-group col-md-5">
          <label >Password</label>
          <input type="password" class="form-control" id="txt"  name="c_pass" required>
        </div>
